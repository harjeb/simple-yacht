...
exports.testMatchmakingSystem = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (testMode === 'basic') {
      ...
      const queueStats = {
        totalInQueue: queueSnapshot.size,
        averageWaitTime: 0,
        eloDistribution: {},
      };
      ...
      queueSnapshot.forEach((doc) => {
        ...
        queueStats.eloDistribution[eloRange] = (queueStats.eloDistribution[eloRange] || 0) + 1;
      });
      ...
      const currentRange = calculateDynamicEloRange(simulatedOpponent);
      const eloDistance = Math.abs((userData.elo || 1200) - simulatedOpponent.elo);
      ...
      return {
        ...
        matchingInfo: {
          currentRange: currentRange,
          eloDistance: eloDistance,
          wouldMatch: eloDistance <= currentRange,
          waitTime: 45,
        },
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
      };
...
exports.getMatchmakingStatistics = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const statistics = {
      queue: {
        totalWaiting: queueSnapshot.size,
        byGameMode: {},
        eloDistribution: {},
        averageWaitTime: 0,
      },
      recentGames: {
        total: recentGamesSnapshot.size,
        completed: 0,
        active: 0,
        averageGameDuration: 0,
        averageEloDistance: 0,
      },
      timestamp: admin.firestore.FieldValue.serverTimestamp(),
    };
    ...
    if (queueSnapshot.size > 0) {
      statistics.queue.averageWaitTime = Math.floor(totalWaitTime / queueSnapshot.size / 1000);
    }

    // 分析最近游戏数据
    let totalGameDuration = 0;
    let totalEloDistance = 0;
    ...
    recentGamesSnapshot.forEach((doc) => {
      ...
      if (data.status === 'completed') {
        ...
        if (data.completedAt && data.createdAt) {
          const duration = data.completedAt.toMillis() - data.createdAt.toMillis();
